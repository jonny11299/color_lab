...
<script>
	...
	function getRenderColor(t, j) {
		return colorStore.hoveredPreview && colorStore.curTailoredIndex === j
			? colorStore.hoveredPreview
			: t.color;
	}
...
<div class="container">
                ...
                border-color: {tailoredIndex === j ? `var(--text)` : `var(--bg)`};
                 border-width: {tailoredIndex === j ? `0px` : `0`}"
							...
							<div class="contrastSwatchRightSide">
								<div
									class="bgswatchrender"
									style="background: {colorStore.tailored[0]?.color}"
								></div>
								<div
									class="bgswatchrender"
									style="background: {colorStore.tailored[1]?.color}"
								></div>
							</div>
						</div>
					{/if}
				...
				<div class="swatchColumn">
					<div class="swatchAndName">
						<div
							...
							style="background: {getRenderColor(t, j)}; --border-color: {getBorderColor(
								getRenderColor(t, j),
							)}"
						...
						<div
							class="swatchName"
							class:nameSelected={tailoredIndex === j}
							onclick={() => selectIndex(j)}
						>
							{t.name}
						</div>
	...
	<div
		...
		onclick={() =>
			window.open("https://git.apcacontrast.com/documentation/APCAeasyIntro.html", "_blank")}
	>
		{contrastMessage}
	</div>
</div>
...
<style>
	...
	.container::-webkit-scrollbar {
		display: none; /* Chrome, Safari, new Edge, Opera */
	}
