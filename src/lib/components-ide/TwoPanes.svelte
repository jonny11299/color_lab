<script>
    ...
    */
	...
	let paneVisible = $state([true, true, true, true, true]);
	...
	let awaitingLaunch = $state([false, true, true, true, true]);
...
<div class="shell" bind:this={shellEl}>
  ...
  -->
	...
	<div class="col" style="width: {100 - leftWidth}%;">
		<div class="pane" class:pane-focused={focus === 2}>
			<!-- svelte-ignore a11y_click_events_have_key_events -->
			<!-- svelte-ignore a11y_no_static_element_interactions -->
...
<style>
  ...
  ──────────────────────────────────────────────────────────────────────── */
	.shell {
		...
		flex-direction: row;
		width: 100%;
		min-height: 100vh; /* adjust to fit your layout — see comment above */
		overflow: visible;
		background: var(--bg);
		color: var(--text);
